...
# Builds the Infernal Invocation r18p runtime for DGX Spark (GB10, aarch64).
# The vLLM overlay remains THREE items: CMake 12.1 arch acceptance,
...
# combined ExLlamaV3 patch (pin unmoved at 704aefd). The B12X composition adds
# one source-level change over r18: paired-M8 FC2 weight-tile reuse, restoring
# the large-M path removed by b4d6c75. Both production K3/K4 expert splits are
# dispatch-, resource-, bit-output-, and latency-gated below. The r15-era
...
release_name=${RELEASE_NAME:-infernal-invocation-r18p-spark-sm121}
release_date=${RELEASE_DATE:-20260820}
revision=${REVISION:-r18p}
composition_root=patches/releases/infernal-invocation-r18p
...
[[ "${revision}" =~ ^r[1-9][0-9]*p?$ ]] || {
  printf 'REVISION must use the rN or rNp form; got %s\n' "${revision}" >&2
  exit 2
}
[[ "${release_date}" =~ ^[0-9]{8}$ ]] || {
...
paired_fc2_patch="${composition_root}/b12x/paired-fc2-restoration.patch"
paired_fc2_sha256="$(jq -er '.source_patches[] | select(.path == "paired-fc2-restoration.patch") | .sha256' "${composition_root}/b12x/integration.lock.json")"
test -f "${paired_fc2_patch}" || {
  printf 'Missing paired-FC2 source patch: %s\n' "${paired_fc2_patch}" >&2
  exit 1
}
echo "${paired_fc2_sha256}  ${paired_fc2_patch}" | sha256sum -c - >/dev/null
...
image=${IMAGE:-localhost/voipmonitor/vllm:infernal-invocation-r18p-spark-sm121-vllm${spark_overlay_tree:0:7}-b12x${B12X_INTEGRATION_TREE:0:7}-fi1ac6942-cu133-torch213-${release_date}}
...
podman build \
  ...
  --build-arg "B12X_PAIRED_FC2_PATCH_SHA256=${paired_fc2_sha256}" \
  ...
  --file Dockerfile.deepseek-ii-r18p-spark-sm121 \
  --tag "${image}" \
  .
...
assert_label local-inference.b12x.paired-fc2.patch_sha256 "${paired_fc2_sha256}"
...
printf 'r18p queue + env hardening gates: PASS\n'
...
# Spark GLM runner contract (FAIL-CLOSED): the II r18p runner file is
# REQUIRED; its render must be nonempty, carry the r18p arguments and exact
...
SPARK_GLM_RUNNER="${SPARK_GLM_RUNNER:-run-glm52-r18p-exl3-tp4-node.sh}"
...
# glm52-exl3 render through the II-owned serve-infernal-invocation.sh.
...
# Production-faithful mixed-Trellis gate for the r18p source change. Decode
# must stay on the unpaired path, prefill must compile and execute paired-M8,
# codegen must not spill, and both observed GLM TP4 K3/K4 expert splits must
# retain the frozen bit-exact outputs and beat bounds that stock r18 fails.
for geometry in "148 108" "192 64"; do
  read -r tier0_experts tier1_experts <<<"${geometry}"
  podman run --rm --device nvidia.com/gpu=all --ipc=host \
    -e B12X_PRINT_COMPILE_PROGRESS=1 \
    -v "$(pwd)/tests/verify_b12x_mixed_trellis_paired_fc2.py:/tmp/gate-paired-fc2.py:ro" \
    --entrypoint /opt/venv/bin/python "${image}" \
    /tmp/gate-paired-fc2.py \
    --label "r18p-${tier0_experts}-${tier1_experts}" \
    --tier0-experts "${tier0_experts}" \
    --tier1-experts "${tier1_experts}" \
    --m 1 32 33 221 222 3072 \
    --warmup 10 --repeats 50 --gate-paired-fc2
done

printf 'SM121 EXL3 + paired-FC2 gate battery: PASS\n'
