...
# Builds the Infernal Invocation r17 runtime for DGX Spark (GB10, aarch64).
...
release_name=${RELEASE_NAME:-infernal-invocation-r17-spark-sm121}
...
spark_overlay_file=${VLLM_SPARK_OVERLAY_FILE:-releases/infernal-invocation-r17-spark/vllm-spark-overlay.patch}
spark_overlay_tree=${VLLM_SPARK_OVERLAY_TREE:-e8793453ede9366f00685e4adb5c671cc8f05060}
...
image=${IMAGE:-localhost/voipmonitor/vllm:infernal-invocation-r17-spark-sm121-vllm${spark_overlay_tree:0:7}-b12x${B12X_INTEGRATION_TREE:0:7}-fi1ac6942-cu133-torch213-${release_date}}
...
podman build \
  ...
  --file Dockerfile.deepseek-ii-r17-spark-sm121 \
  --tag "${image}" \
  .
...
printf 'r17 queue + env hardening gates: PASS\n'
...
# Spark GLM runner contract (FAIL-CLOSED): the II r17 runner file is
# REQUIRED; its render must be nonempty, carry the r17 arguments, and
# contain neither NCCL/cache-identity overrides nor any of the four GG
# EXL3 tuning variables.
SPARK_GLM_RUNNER="${SPARK_GLM_RUNNER:-run-glm52-r17-exl3-tp4-node.sh}"
...
if grep -qE -- '-e (LD_PRELOAD|VLLM_NCCL_SO_PATH|XDG_CACHE_HOME|VLLM_CACHE_DIR|TRITON_CACHE_DIR|TORCHINDUCTOR_CACHE_DIR|TORCH_EXTENSIONS_DIR|FLASHINFER_WORKSPACE_BASE|TVM_CACHE_DIR|TILELANG_CACHE_DIR)=' <<<"${runner_render}"; then
  echo "Spark GLM runner renders forbidden NCCL/cache overrides" >&2; exit 1
fi
