...
#     (kernel_warmup), and rewrote the W4A16 tile admission (SI #112) so
#     the ultra-tile bypass is dropped — a16 is re-verified at the gate.
#   - SparkInfer and LMCache compositions are r24 VERBATIM.
#   - Dockerfile.vllm-sparkinfer-cu132: r24's Dockerfile + the SM121 port
#     (arch args, multiarch paths, sbsa fix, per-stage SHELL, DeepGEMM
#     hook, NCCL preloader fix, tvm-ffi build pin, sha-fetch clones,
#     SKIP_EXLLAMAV3 incl. the new /opt/exllamav3-python packaging).
#
# Composed trees (locks are authoritative):
#   vLLM       41ae549887b0f78bb27a8b612184bdbe66ba7cc9 (r24 f5981f14 + SM121
#              + flashinfer frozen-q_len guard: persistent decode wrapper
#              reuse requires decode_q_len == planned 1+K; fixes
#              EngineDeadError 'planned with 6, got 8|5' on dflash)
#   SparkInfer 2b9bf2a4d15770c0c23e19cc13a75843f2f0a995 (r24 verbatim)
#   LMCache    9a05c8818bae48d15b79c7e876418bb813c08cd0 (r24 verbatim)
#
...
release_dir="patches/releases/gilded-gnosis-v20p3p1-spark-r24"
...
export DATE_TAG="${DATE_TAG:-20260803}"
export IMAGE="${IMAGE:-voipmonitor/vllm:gilded-gnosis-v20p3p1-spark-sm121-vllm${VLLM_INTEGRATION_TREE:0:7}-si${SPARKINFER_INTEGRATION_TREE:0:7}-fi7ad08da-cu132-${DATE_TAG}}"
export VLLM_BUILD_VERSION="${VLLM_BUILD_VERSION:-0.11.2.dev280+gilded.gnosis.v20p3p1.spark.vllm${VLLM_INTEGRATION_TREE:0:7}.si${SPARKINFER_INTEGRATION_TREE:0:7}.fi7ad08da.sm121.cu132.${DATE_TAG}}"
...
# v20p3.1 frozen-q_len invariant gate: threshold(1+2K) vs planned shape(1+K)
# derivation, q_len=1+K -> persistent wrapper, every q_len in {1..1+2K}\{1+K}
# -> dynamic fallback, q_len>ceiling -> prefill classification. Guards the
# flashinfer.py predicate patch; e2e reproducers run at first serving boot.
podman run --rm --device nvidia.com/gpu=all --security-opt label=disable \
  -v "$(pwd)/tests:/build-tests:ro" \
  --entrypoint /opt/venv/bin/python "${IMAGE}" \
...
# a16 wiring: the ultra-tile bypass was dropped (upstream rewrote W4A16
# admission); a real a16 BOOT on a GB10 is the post-build acceptance test.
a16_cfg="$(ds4_dry_run -e MODE=dspark -e BACKEND=b12x-a16 -e TP_SIZE=2 -e DSPARK_TOKENS=7)"
grep -q 'backend=b12x-a16' <<<"${a16_cfg}"

echo "v20p3p1 build + gates: PASS ${IMAGE}"
