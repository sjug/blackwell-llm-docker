...
capture-safe K6/MCG kernel is not admitted (capability gate is (12, 0)),
so `_b12x_trellis_linear` runs the generic W4A16 kernel at small M - the
exact configuration every GLM capture size (4..32) uses in R7 serving.
Without the planner fix, capture raises on the one-float scratch buffer.

Shapes are the actual r34 TP4 shared-expert projections for GLM-5.2
