...
packed `trellis`/`suh`/`svh` - then verifies:
  ...
  2. The ExLlamaV3 extension's fused linear (ext.BC_LinearEXL3) with an
     ...
     ExLlamaV3 execution contract, NOT the b12x serving route; the r17
     #221 CuTe K6/MCG small-M path IS admissible on SM121 (per-weight
     launch binding, m <= 16) and its dispatch + CUDA-graph capture are
     proven by verify_b12x_trellis_graph_capture.py.
...
def encode():
    ...
    encode.proxy_error = float(proxy_error)
...
if not _math.isfinite(encode.proxy_error):
    failures.append(f"encoder proxy error non-finite: {encode.proxy_error}")
print(f"encoder proxy error: {encode.proxy_error:.6f}")
...
# r17 dispatch fact: the #221 CuTe K6/MCG small-M path is admissible on
# SM121 (per-weight launch binding, m <= 16); full graph coverage lives in
# verify_b12x_trellis_graph_capture.py. This gate asserts the stack-level
# support predicate and the incompatible-env hardening.
from b12x.gemm.trellis_linear import api as _tl_api  # noqa: E402

cap = torch.cuda.get_device_capability(DEV)
supported = bool(_tl_api.is_supported(DEV))
print(f"capability={cap} trellis_linear api supported={supported}")
if not supported:
    failures.append(f"b12x trellis_linear api not supported on {cap}")
