...
packed `trellis`/`suh`/`svh` - then verifies:
  ...
  2. The ExLlamaV3 extension's fused linear (ext.BC_LinearEXL3) with an
     ...
     production TP4 shared-expert shapes; full-model serving qualification
     remains for the R7 window. This gate asserts the dispatch fact so the
...
# verify_b12x_trellis_graph_capture.py and fully qualified in the R7
# serving window.
from b12x.moe._shared.kernels.w4a16.kernel import _use_k6_mcg_small  # noqa: E402

cap = torch.cuda.get_device_capability(DEV)
admitted = _use_k6_mcg_small(
